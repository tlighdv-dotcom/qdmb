...
export function isConfigured(config = getConfig()) {
  return Boolean(
    config.SUPABASE_URL &&
    config.SUPABASE_KEY &&
    !String(config.SUPABASE_URL).startsWith('YOUR_') &&
    !String(config.SUPABASE_KEY).startsWith('YOUR_')
  );
}
...
export function createDbClient() {
  ...
  if (!isConfigured(config)) {
    throw new Error('Supabase chưa được cấu hình. Hãy cập nhật config.js.');
  }
